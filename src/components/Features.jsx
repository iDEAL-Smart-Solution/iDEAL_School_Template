...
const Features = ({ schoolData }) => {
  ...
  const name = schoolData?.name || DEFAULT_LANDING_PAGE.name;
  ...
  return (
    <section id="features" ref={sectionRef} data-reveal className={`py-16 sm:py-20 lg:py-24 bg-slate-50 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div
            className="inline-block mb-4 rounded-full px-3 py-1 text-sm font-semibold"
            style={{ backgroundColor: `${resolveColor(theme_color, DEFAULT_LANDING_PAGE.theme_color)}29`, color: secondary }}
          >
            Why Choose Us
          </div>
